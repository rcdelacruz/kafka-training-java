...
public class InteractiveProducer {
    ...
    public void interactiveMode() {
        ...
        while (true) {
            ...
            producer.send(record, (metadata, exception) -> {
                if (exception == null) {
                    String keyInfo = finalKey != null ? "Key: '" + finalKey + "' → " : "No key → ";
                    logger.info("{}Message: '{}' → Partition: {}, Offset: {}",
                        keyInfo, finalMessage, metadata.partition(), metadata.offset());
                } else {
                    logger.error("Error sending message: {}", exception.getMessage());
                }
            });
    ...
    public void demonstrateDeliverySemantics() {
        ...
        atMostOnceProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092");
        atMostOnceProps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class.getName());
        atMostOnceProps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, StringSerializer.class.getName());
        ...
        atLeastOnceProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092");
        atLeastOnceProps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class.getName());
        atLeastOnceProps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, StringSerializer.class.getName());
